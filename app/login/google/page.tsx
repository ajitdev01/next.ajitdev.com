    ...
    <div className="relative min-h-screen bg-slate-50 text-slate-900 selection:bg-slate-900 selection:text-white antialiased flex flex-col justify-between overflow-x-clip">
      ...
      <main className="relative z-10 mx-auto flex w-full max-w-md flex-col items-center justify-center px-4 py-8">
        <div className="w-full rounded-2xl border border-slate-200/90 bg-white p-6 sm:p-8 shadow-xl shadow-slate-200/40">
          ...
          {/* Server Action Form: Official Google OAuth Login */}
          {!session?.user ? (
            <form
              action={async () => {
                "use server";
            ...
            >
              <button
                type="submit"
                className="group relative flex w-full items-center justify-center gap-3 rounded-xl border border-slate-300 bg-white px-4 py-3 text-xs font-bold text-slate-700 shadow-xs transition duration-200 hover:border-slate-400 hover:bg-slate-50 hover:text-slate-900 active:scale-98 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 cursor-pointer"
              >
                {/* Official Google G Icon */}
            ...
            >
              <button
                type="submit"
                className="flex w-full items-center justify-center gap-2 rounded-xl border border-rose-200 bg-rose-50/70 px-4 py-2.5 text-xs font-semibold text-rose-700 hover:bg-rose-100 transition active:scale-98 cursor-pointer"
              >
          ...
          {/* Cloud & Atlas Storage Notice */}
          <div className="mt-6 rounded-xl border border-slate-100 bg-slate-50/80 p-3 text-[11px] text-slate-600">
            <div className="flex items-center gap-2 font-semibold text-slate-800">
              <Database className="h-3.5 w-3.5 text-emerald-600" />
              <span>MongoDB Atlas Cloud Persistence</span>
            </div>
            <p className="mt-1 text-slate-500 leading-relaxed text-[10px]">
              Upon sign-in, your authenticated profile is safely stored in our MongoDB Atlas cluster via secure Auth.js callbacks.
            </p>
          </div>
